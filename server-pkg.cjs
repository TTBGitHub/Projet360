...
// Determine the correct dist directory path
// When running in pkg, __dirname points to the snapshot root
// When running normally, __dirname is the project root
let DIST_DIR;

if (process.pkg) {
  // Running as packaged executable - files are in the snapshot
      ...
      if (err.code === 'ENOENT') {
        ...
        fs.readFile(path.join(DIST_DIR, 'index.html'), (err2, data2) => {
          if (err2) {
            ...
            res.end('<h1>404 - File not found</h1><p>DIST_DIR: ' + DIST_DIR + '</p>');
          } else {
